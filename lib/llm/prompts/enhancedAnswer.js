...
export function getEnhancedAnswerPrompt(language, { text, question, activeGaPair = null }) {
  const gaPromptText = getGAPrompt(language, { activeGaPair });
  const prompt = (language === 'en' ? ENHANCED_ANSWER_PROMPT_EN : ENHANCED_ANSWER_PROMPT)
    .replaceAll('{{gaPrompt}}', gaPromptText)
    .replaceAll('{{text}}', text)
    .replaceAll('{{question}}', question);
  console.log(666, prompt);
  return prompt;
}
