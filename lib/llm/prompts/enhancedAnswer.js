...
export async function getEnhancedAnswerPrompt(language, { text, question, activeGaPair = null }, projectId = null) {
  ...
  const promptKey = getPromptKey(language, 'ENHANCED_ANSWER_PROMPT');
  const defaultPrompt = language === 'en' ? ENHANCED_ANSWER_PROMPT_EN : ENHANCED_ANSWER_PROMPT;
  ...
  if (projectId) {
    try {
      prompt = await getPromptContent(projectId, 'enhancedAnswer', promptKey, langCode, defaultPrompt);
    } catch (error) {
