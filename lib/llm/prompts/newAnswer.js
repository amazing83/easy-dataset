...
export async function getNewAnswerPrompt(language, { question, answer, cot, advice, chunkContent }, projectId = null) {
  const promptKey = getPromptKey(language, 'NEW_ANSWER_PROMPT');
  const defaultPrompt = language === 'en' ? NEW_ANSWER_PROMPT_EN : NEW_ANSWER_PROMPT;
  ...
  if (projectId) {
    try {
      prompt = await getPromptContent(projectId, 'newAnswer', promptKey, langCode, defaultPrompt);
    } catch (error) {
  ...
  const result = prompt
    .replaceAll('{{chunkContent}}', chunkContent || '')
    .replaceAll('{{question}}', question)
    .replaceAll('{{answer}}', answer)
    .replaceAll('{{cot}}', cot)
    .replaceAll('{{advice}}', advice);

  console.log(1010, result);
  return result;
}
