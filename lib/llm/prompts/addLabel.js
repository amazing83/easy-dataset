import { getPromptContent, getPromptKey, getLanguageFromKey } from '../common/prompt-loader';
...
export async function getAddLabelPrompt(language, { label, question }, projectId = null) {
  const promptKey = getPromptKey(language, 'ADD_LABEL_PROMPT');
  const defaultPrompt = language === 'en' ? ADD_LABEL_PROMPT_EN : ADD_LABEL_PROMPT;
  ...
  if (projectId) {
    try {
      prompt = await getPromptContent(projectId, 'addLabel', promptKey, langCode, defaultPrompt);
    } catch (error) {
  ...
  const result = prompt.replaceAll('{{label}}', label).replaceAll('{{question}}', question);
