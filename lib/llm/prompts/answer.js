...
export async function getAnswerPrompt(language, { text, question }, projectId = null) {
  const promptKey = getPromptKey(language, 'ANSWER_PROMPT');
  const defaultPrompt = language === 'en' ? ANSWER_PROMPT_EN : ANSWER_PROMPT;
  ...
  if (projectId) {
    try {
      prompt = await getPromptContent(projectId, 'answer', promptKey, langCode, defaultPrompt);
    } catch (error) {
  ...
  const result = prompt.replaceAll('{{text}}', text).replaceAll('{{question}}', question);
