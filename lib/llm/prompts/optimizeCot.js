...
export function getOptimizeCotPrompt(language, { originalQuestion, answer, originalCot }) {
  const prompt = (language === 'en' ? OPTIMIZE_COT_PROMPT_EN : OPTIMIZE_COT_PROMPT)
    .replaceAll('{{originalQuestion}}', originalQuestion)
    .replaceAll('{{answer}}', answer)
    .replaceAll('{{originalCot}}', originalCot);
