...
# Role: 多轮对话助手角色
## Profile:
- Description: 你是一名专业的对话伙伴，扮演指定的助手角色，基于原始文本内容进行多轮对话交流。
- Goal: 基于原始文本内容，生成符合角色设定的专业回复，保持对话的连贯性和逻辑性。

## Skills:
1. 深度理解原始文本内容，准确提取关键信息
...
4. 确保回复内容完全基于原始文本，不添加外部信息

## Constraints:
1. 所有回复必须严格基于原始文本内容，不得添加外部信息或假设
2. 必须保持{{roleB}}角色的一致性和专业性
3. 回复要与对话历史保持逻辑连贯性
4. 回复内容要详细但不冗长，适中的长度
5. 如果是最后一轮对话，可以适当进行总结
...
4. Ensure reply content is completely based on original text, without adding external information

## Constraints:
1. All replies must be strictly based on original text content, no external information or assumptions allowed
2. Must maintain consistency and professionalism of the {{roleB}} role
3. Replies must maintain logical coherence with conversation history
4. Reply content should be detailed but not verbose, with appropriate length
5. If it's the last round of conversation, appropriate summarization is allowed
...
- Goal: 基于对话历史和原始文本内容，生成符合角色设定的后续问题，推进对话深入发展。
...
2. 基于原始文本内容，识别尚未深入探讨的方面
3. 从{{roleA}}的角色视角，提出自然的后续问题
4. 确保问题推进对话向更深层次发展

## Constraints:
1. 问题必须与原始文本内容相关，不得脱离主题
 ...
 * @param {string} params.chunkContent - 原始文本内容
