...
export async function getQuestionPrompt(
  ...
  const gaPromptCheck = gaPromptText
    ? language === 'en'
      ...
      : '- 问题风格与指定的体裁受众匹配'
    : '';
  const promptKey = getPromptKey(language, 'QUESTION_PROMPT');
  const defaultPrompt = language === 'en' ? QUESTION_PROMPT_EN : QUESTION_PROMPT;
  ...
  if (projectId) {
    try {
      prompt = await getPromptContent(projectId, 'question', promptKey, langCode, defaultPrompt);
    } catch (error) {
